...
//  Nasa
//
//  Created by Jhon Ospino Bernal on 8/06/20.
...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        //        window?.rootViewController = UIViewController()    // In ViewController.swift
        //        window?.rootViewController?.view.backgroundColor = .yellow  // So we can see it
        //        window?.makeKeyAndVisible()
        
        window?.windowScene = windowScene
        let flexibleHeaderVC = FlexibleHeaderContainerViewController()
        window?.rootViewController = flexibleHeaderVC
        window?.makeKeyAndVisible()
